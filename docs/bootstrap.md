...
| claude.ai web, mobile, voice — and Claude Code signed in with the same claude.ai account (the connector is mirrored into those sessions — `claude mcp list` shows it as `claude.ai brain-remote` when Claude Code is signed in; an expired login hides it) | the **brain-remote custom connector**, added once on claude.ai | **No** — the connector is the registration. |
