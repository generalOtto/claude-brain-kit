...
| claude.ai web, mobile, voice — and Claude Code signed in with the same claude.ai account (the connector is mirrored into those sessions: the `brain_*` tools are simply present; note `claude mcp list` does NOT list mirrored connectors) | the **brain-remote custom connector**, added once on claude.ai | **No** — the connector is the registration. |
